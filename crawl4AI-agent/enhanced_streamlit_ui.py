...
from combined_expert import combined_expert, CombinedDeps
...
async def run_agent_with_streaming(user_input: str, expert_type: str):
    ...
    Args:
        ...
        expert_type: Which expert to use ('mcp', 'myfitnesspal', or 'combined')
    """
...
async def main():
    ...
    # Expert selection with Combined option
    expert_type = st.sidebar.radio(
        "Select Documentation Expert:",
        ["Combined MCP + MyFitnessPal", "Model Context Protocol (MCP)", "MyFitnessPal API"],
        key="expert_selection"
    )
    
    # Convert selection to key
    if expert_type == "Model Context Protocol (MCP)":
        expert_key = "mcp"
        expert_description = "Ask any question about the Model Context Protocol (MCP), its specifications, implementation details, or best practices."
    elif expert_type == "MyFitnessPal API":
        expert_key = "myfitnesspal"
        expert_description = "Ask any question about the MyFitnessPal API, its endpoints, authentication methods, data models, and usage examples."
    else:
        expert_key = "combined"
        expert_description = "Ask questions that combine MCP with MyFitnessPal API knowledge. This expert can help you build an MCP server that uses MyFitnessPal APIs to create a nutrition AI agent."
    
    # Display expert info with custom styling for combined expert
    if expert_key == "combined":
        st.markdown("### Combined MCP + MyFitnessPal Expert")
        st.write(expert_description)
        st.markdown("---")
    else:
        st.write(expert_description)
    ...
    if expert_key == "mcp":
        placeholder_text = "What questions do you have about the Model Context Protocol?"
    elif expert_key == "myfitnesspal":
        placeholder_text = "What questions do you have about the MyFitnessPal API?"
    else:
        placeholder_text = "Ask how to combine MCP and MyFitnessPal for your nutrition AI agent..."
