...
async def auto_discover_crawl(start_url: str, source_tag: str, url_filter: str = None, max_pages: int = 100, skip_existing: bool = True):
    """
    ...
    if skip_existing:
        ...
        print(f"Skipping {len(already_crawled)} already crawled URLs")
    ...
    try:
        ...
        while to_crawl and pages_crawled < max_pages:
            ...
            # Skip if already in database (from previous runs)
            if skip_existing and current_url in already_crawled:
                print(f"Skipping already crawled URL: {current_url}")
...
async def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python crawl_auto_discover_incremental.py <start_url> <source_tag> [url_filter] [max_pages] [--force]")
        print("Example: python crawl_auto_discover_incremental.py https://myfitnesspalapi.com/docs/ myfitnesspal_docs '/docs/' 50")
    ...
    await auto_discover_crawl(start_url, source_tag, url_filter, max_pages, skip_existing)
